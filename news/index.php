...
function feed() {
    ...
    if(!file_exists(basePath.'/rss.xml') || time() - filemtime(basePath.'/rss.xml') > feed_update_time) {
        ...
        $qry = common::$sql['default']->select("SELECT `id`,`autor`,`datum`,`titel`,`text` FROM `{prefix_news}` WHERE `intern` = 0 AND `public` = 1 ORDER BY `datum` DESC LIMIT 15;");
        if(common::$sql['default']->rowCount()) {
            foreach($qry as $get) {
                ...
                $feed .= "\r\n";
                $feed .= '    <author>'.convert_feed(common::data('nick', $get['autor'])).'</author>';
                $feed .= "\r\n";
