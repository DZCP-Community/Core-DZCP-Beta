...
if(defined('_Tutorials')) {
    if(common::$do == 'edit') {
        $get = common::$sql['default']->fetch("SELECT * FROM `{prefix_tutorials_comments}` WHERE `id` = ?;", [(int)($_GET['cid'])]);
        ...
        $get_id = common::cnt("{prefix_tutorials_comments}", " WHERE `tutorial` = ?;","id", [(int)($_GET['id'])])+1;
    ...
    $index = $smarty->fetch('file:['.common::$tmpdir.']'.$dir.'/comments/comments_show.tpl');
    $smarty->clearAllAssign();
