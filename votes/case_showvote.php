...
if(defined('_Votes')) {
    $get = common::$sql['default']->fetch("SELECT `id`,`intern`,`closed` FROM `{prefix_votes}` WHERE `id` = ?;",array(intval($_GET['id'])));
    if(!$get['intern'] || ($get['intern'] && common::$chkMe)) {
        ...
        if(common::$chkMe == 4 || $get['closed'] || common::permission('votesadmin') || common::$sql['default']->rows("SELECT `id` FROM `{prefix_ipcheck}` "
                . "WHERE `user_id` = ? AND `what` = ?;",array(common::$userid,'vid_'.$get['id']))) {
            foreach ($qryv as $getv) {
                $class = ($color % 2) ? "contentMainSecond" : "contentMainFirst"; $color++;
                $show .= show($dir."/voted_show", array("user" => $getv['user_id'] ? common::autor($getv['user_id']) : _gast,
