...
switch (common::$do) {
    ...
    case 'new':
        ...
        if(!notification::is_success()) {
            ...
            $smarty->assign('error',$errortable);
            $show = $smarty->fetch('file:['.common::$tmpdir.']'.$dir.'/changelog_form.tpl');
            $smarty->clearAllAssign();
        }
        break;
    default:
        $qry = common::$sql['default']->select("SELECT * FROM `{prefix_addon_mods}`;"); $color = 0; $show = '';
        foreach($qry as $get) {
            ...
            $smarty->assign('title', stringParser::decode($get['title']));
            $smarty->assign('mid',stringParser::decode($get['mid']));
            $smarty->assign('dzcp_edition',stringParser::decode($get['dzcp_edition']) == 'dev' ? 'Development' : 'Live/Stable');
            $smarty->assign('dzcp_version',stringParser::decode($get['dzcp_version']));
            $smarty->assign('version',stringParser::decode($get['version']));
            $smarty->assign('class',$class);
            $smarty->assign('delete',$delete);
            $show .= $smarty->fetch('file:['.common::$tmpdir.']'.$dir.'/changelog_show.tpl');
            $smarty->clearAllAssign();;
        }

        if(empty($show)) {
            $smarty->caching = false;
            $smarty->assign('colspan',7);
        ...
        $show = $smarty->fetch('file:['.common::$tmpdir.']'.$dir.'/changelog.tpl');
        $smarty->clearAllAssign();
        break;
}