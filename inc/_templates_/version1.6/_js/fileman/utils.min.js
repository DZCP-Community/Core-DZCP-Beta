...
RoxyUtils.GetFileIcon = function (path) {
    ret = 'images/fileman/filetypes/unknown.png';//'images/filetypes/file_extension_' + RoxyUtils.GetFileExt(path).toLowerCase() + '.png';
    if (fileTypeIcons[RoxyUtils.GetFileExt(path).toLowerCase()]) {
        ret = 'images/fileman/filetypes/' + fileTypeIcons[RoxyUtils.GetFileExt(path).toLowerCase()];
    }
...
RoxyUtils.LoadLang = function () {
    ...
    if (RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == 'auto') {
        if (RoxyUtils.GetUrlParam('langCode')) {
            langUrl = 'lang/fileman/' + RoxyUtils.GetUrlParam('langCode').substr(0, 2).toLowerCase() + '.json';
        }
        else {
            var language = window.navigator.userLanguage || window.navigator.language;
            langUrl = 'lang/fileman/' + language.substr(0, 2) + '.json';
        }
        ...
        if (RoxyFilemanConf.LANG) {
            langUrl = 'lang/fileman/' + RoxyFilemanConf.LANG.substr(0, 2).toLowerCase() + '.json';
            if (!RoxyUtils.FileExists(langUrl))
                langUrl = '';
        }
    }
    if (!langUrl)
        langUrl = 'lang/fileman/en.json';
