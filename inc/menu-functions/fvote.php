...
function fvote($id, $ajax=false) {
    $get = common::$sql['default']->fetch("SELECT `id`,`closed`,`titel` FROM `{prefix_votes}` WHERE `id` = ? ".(common::permission("votes") ? ";" : " AND `intern` = 0;"),array(intval($id)));
    if(common::$sql['default']->rowCount()) {
        $results = ''; $votebutton = '';
        $qryv = common::$sql['default']->select("SELECT `id`,`stimmen`,`sel` FROM `{prefix_vote_results}` WHERE `vid` = ? ORDER BY `id` ASC;",array($get['id']));
        if(common::$sql['default']->rowCount()) {
            foreach($qryv as $getv) {
                $stimmen = common::sum('{prefix_vote_results}', " WHERE `vid` = ?", "stimmen",array($get['id']));
                if($stimmen != 0) {
        ...
        $getf = common::$sql['default']->fetch("SELECT `id`,`kid` FROM `{prefix_forumthreads}` WHERE `vote` = ?;",array($get['id']));
