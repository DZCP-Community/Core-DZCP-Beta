...
function vote($ajax = false) {
    ...
    if(common::$sql['default']->rowCount()) {
        if(!$get['intern'] || common::$chkMe >= 1) {
            $qryv = common::$sql['default']->select("SELECT `id`,`stimmen`,`sel` FROM `{prefix_vote_results}` WHERE `vid` = ? ORDER BY `what`;",array($get['id']));
            $results = '';
            foreach($qryv as $getv) {
                $ipcheck = !common::count_clicks('vote',$get['id'],0,false);
                $stimmen = common::sum("{prefix_vote_results}", " WHERE `vid` = '".$get['id']."'", "stimmen",array($get['id']));
                if($stimmen != 0) {
