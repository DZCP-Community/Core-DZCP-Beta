...
class sfs extends common {
    ...
    public static function check() {
        ...
        if(!self::validateIpV4Range(self::$userip, '[192].[168].[0-255].[0-255]') && !self::validateIpV4Range(self::$userip, '[127].[0].[0-255].[0-255]') &&
            !self::validateIpV4Range(self::$userip, '[10].[0-255].[0-255].[0-255]') && !self::validateIpV4Range(self::$userip, '[172].[16-31].[0-255].[0-255]')) {
            $get = self::$sql['default']->fetch("SELECT * FROM `{prefix_ipban}` WHERE `ip` = ? LIMIT 1;",array(self::$userip));
            if(self::$sql['default']->rowCount()) {
                if((time()-$get['time']) > (2*86400) && $get['enable']) {
                    self::get(array('ip' => self::$userip)); //Array ( [success] => 1 [ip] => Array ( [lastseen] => 2013-04-26 19:57:51 [frequency] => 1327 [appears] => 1 [confidence] => 99.89 ) )
                    ...
                    if(is_array($stopforumspam) && array_key_exists('success', $stopforumspam) && $stopforumspam['success']) {
                        ...
                        if($stopforumspam['appears'] == '1' && ($stopforumspam['confidence'] >= self::$confidence && $stopforumspam['frequency'] >= self::$frequency) && self::$autoblock) {
                            ...
                            self::$sql['default']->delete("DELETE FROM `{prefix_counter_ips}` WHERE `ip` = ?;",array(self::$userip));
                            self::$sql['default']->delete("DELETE FROM `{prefix_counter_whoison}` WHERE `ip` = ?;",array(self::$userip));
                            self::$sql['default']->delete("DELETE FROM `{prefix_iptodns}` WHERE `ip` = ?;",array(self::$userip));
                            self::$blockuser = true;
                        } else {
                ...
                if(is_array($stopforumspam) && array_key_exists('success', $stopforumspam) && $stopforumspam['success']) {
                    ...
                    if($stopforumspam['appears'] == '1' && $stopforumspam['confidence'] >= self::$confidence && $stopforumspam['frequency'] >= self::$frequency && self::$autoblock) {
                        ...
                        self::$sql['default']->delete("DELETE FROM `{prefix_counter_whoison}` WHERE `ip` = ?;",array(self::$userip));
                        self::$sql['default']->delete("DELETE FROM `{prefix_iptodns}` WHERE `ip` = ?;",array(self::$userip));
                        self::$sql['default']->insert("INSERT INTO `{prefix_ipban}` SET `ip` = ?, `time` = ?, `typ` = 1, `data` = ?;",
                                array(self::$userip,time(),serialize($stopforumspam))); //Banned
                        self::$blockuser = true;
                    } else {
                        $stopforumspam['banned_msg'] = '';
                        self::$sql['default']->insert("INSERT INTO `{prefix_ipban}` SET `ip` = ?, `time` = ?,`typ` = 0, `data` = ?;",
                                array(self::$userip,time(),serialize($stopforumspam))); //Add to DB
                        self::$blockuser = false;
                    }
                }
            }
